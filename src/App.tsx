...
const App: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    const newMessage: ChatMessage = { id: Date.now().toString(), role: 'user', text: input, timestamp: Date.now(), type: 'text' };
    ...
    try {
      if (mode === AppMode.IMAGE) {
        ...
        const responseText = await geminiService.sendMessage(historyForApi, newMessage.text, null, language);
        const botMsg: ChatMessage = { id: (Date.now() + 1).toString(), role: 'model', text: responseText || "No response received.", timestamp: Date.now(), type: 'text' };
        updateCurrentSession([...updatedMessages, botMsg]);
      }
    } catch (error: any) {
      console.error(error);
      const errorText = error.message?.includes('403') || error.message?.includes('key') 
          ? "⚠️ API Key Error: The provided API Key is invalid or has leaked. Please check your environment variables." 
          : "⚠️ Connection Error: Failed to connect to Burnit AI.";
  ...
  const renderUniqueAiVisual = (sizeClass: string = "w-32 h-32") => (
      <div className={`relative flex items-center justify-center p-8 ${sizeClass}`}>
         ...
         <div className="absolute -bottom-6 bg-black text-[10px] text-burnit-cyan px-3 py-1 border border-burnit-cyan/50 font-mono tracking-widest whitespace-nowrap">BURNIT AI::ONLINE</div>
      </div>
  );
      ...
      <div className={`fixed inset-y-0 left-0 z-[100] w-72 bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-xl border-r border-gray-200 dark:border-white/10 transition-transform duration-300 md:relative md:translate-x-0 flex flex-col h-full shadow-2xl md:shadow-none ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 flex flex-col gap-6 shrink-0 mt-12 md:mt-0"> 
          <div className="flex items-center justify-between">
            ...
            <button className="md:hidden text-gray-500 hover:text-white" onClick={() => setIsSidebarOpen(false)}><X size={28} /></button>
          </div>
          <nav className="flex flex-col gap-2">
            <button onClick={createNewChat} className="flex items-center justify-start gap-3 p-3.5 rounded-xl transition-all bg-gradient-to-r from-burnit-cyan/20 to-burnit-red/20 border border-burnit-cyan/30 hover:border-burnit-cyan text-black dark:text-white group">
            ...
            <button onClick={() => { setMode(AppMode.CHAT); setIsSidebarOpen(false); }} className={`flex items-center justify-start gap-3 p-3 rounded-xl transition-all ${mode === AppMode.CHAT ? 'bg-burnit-cyan/20 text-burnit-cyan' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400'}`}>
              <MessageSquare size={20} /><span className="font-medium">Chat</span>
            </button>
            <button onClick={() => { setMode(AppMode.IMAGE); setIsSidebarOpen(false); }} className={`flex items-center justify-start gap-3 p-3 rounded-xl transition-all ${mode === AppMode.IMAGE ? 'bg-purple-500/20 text-purple-600 dark:text-purple-400' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400'}`}>
              <ImageIcon size={20} /><span className="font-medium">Image Gen</span>
            </button>
            <button onClick={() => { setMode(AppMode.LIVE); setIsSidebarOpen(false); }} className={`flex items-center justify-start gap-3 p-3 rounded-xl transition-all ${mode === AppMode.LIVE ? 'bg-burnit-red/20 text-burnit-red' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400'}`}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 py-2">
           ...
           <div className="flex flex-col gap-1">
               {sessions.map(session => (
                   <button key={session.id} onClick={() => switchSession(session.id)} className={`flex items-center justify-start gap-3 p-2.5 rounded-lg transition-all text-sm truncate w-full ${currentSessionId === session.id && mode === AppMode.CHAT ? 'bg-black/5 dark:bg-white/10 text-black dark:text-white border border-gray-200 dark:border-white/5' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}>
        ...
        <div className="p-5 flex flex-col gap-2 shrink-0 border-t border-gray-200 dark:border-white/10 mt-auto bg-gray-50/50 dark:bg-black/20">
           <button onClick={() => { setMode(AppMode.PROFILE); setIsSidebarOpen(false); }} className={`flex items-center justify-start gap-3 p-3 rounded-xl transition-all ${mode === AppMode.PROFILE ? 'bg-black/10 dark:bg-white/10 text-black dark:text-white' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400'}`}>
           ...
           <button onClick={() => { setMode(AppMode.SETTINGS); setIsSidebarOpen(false); }} className={`flex items-center justify-start gap-3 p-3 rounded-xl transition-all ${mode === AppMode.SETTINGS ? 'bg-black/10 dark:bg-white/10 text-black dark:text-white' : 'hover:bg-black/5 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400'}`}>
      ...
      {mode === AppMode.SETTINGS ? (
          <div className="flex-1 flex flex-col h-full bg-white/50 dark:bg-black/50 overflow-hidden relative z-10">
              <div className="h-16 shrink-0 flex items-center px-4 border-b border-gray-200 dark:border-white/10 bg-white/80 dark:bg-black/20 backdrop-blur-md z-40 gap-4">
                   <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-black dark:text-white"><Menu size={24} /></button>
                ...
                <div className="max-w-2xl bg-white dark:bg-[#111] p-6 md:p-8 rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl mx-auto">
                    <div className="flex flex-col items-center mb-8">
                        <div className="relative group cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="text-white w-8 h-8 drop-shadow-lg" /></div>
                        </div>
          ...
          isLiveConnected ? (
             <div className="flex-1 flex flex-col h-full bg-gray-900 dark:bg-[#0a0a0a] p-2 md:p-4 gap-4 overflow-hidden relative">
                <button onClick={() => setIsSidebarOpen(true)} className="absolute top-6 left-6 z-[60] p-3 text-white bg-black/60 rounded-full border border-white/20 hover:bg-black/80"><Menu size={24} /></button>
                <div className="flex-1 flex flex-col md:flex-row gap-4 h-full overflow-hidden">
                    <div className="flex-1 bg-black rounded-2xl border border-white/10 relative overflow-hidden flex flex-col items-center justify-center min-h-[40%]">
                        ...
                        <div className="relative flex flex-col items-center justify-center w-full h-full">{renderUniqueAiVisual("w-48 h-48 md:w-64 md:h-64")}</div>
                    </div>
             ...
             <div className="flex-1 flex items-center justify-center p-6 relative overflow-hidden h-full">
                <div className="absolute top-4 left-4 z-50"><button onClick={() => setIsSidebarOpen(true)} className="p-3 bg-white/10 rounded-full text-white backdrop-blur-md border border-white/20 hover:bg-white/20"><Menu size={24} /></button></div>
                ...
                <div className="z-10 text-center max-w-lg flex flex-col items-center">
                    ...
                    <button onClick={startLiveSession} className="group relative px-8 py-4 bg-black dark:bg-white text-white dark:text-black font-bold text-xl rounded-full overflow-hidden transition-all hover:scale-105 hover:shadow-[0_0_40px_rgba(255,42,42,0.3)] dark:hover:shadow-[0_0_40px_rgba(255,255,255,0.3)]">
        ...
        <div className="flex-1 flex flex-col h-full relative overflow-hidden">
          <header className="h-16 shrink-0 flex items-center justify-between px-4 md:px-6 border-b border-gray-200 dark:border-white/10 bg-white/80 dark:bg-black/20 backdrop-blur-md z-10 transition-colors">
            <div className="flex items-center gap-2">
                <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 text-black dark:text-white"><Menu /></button>
          ...
          <div className="shrink-0 p-4 md:p-6 bg-gradient-to-t from-gray-100 via-gray-100/80 to-transparent dark:from-black dark:via-black/80 dark:to-transparent">
            <div className="relative flex items-center gap-2 max-w-4xl mx-auto bg-white dark:bg-white/5 p-2 rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl">
              <button onClick={() => alert("Coming Soon!")} className="p-3 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 rounded-xl transition-colors"><Paperclip size={20} /></button>
              ...
              <button onClick={handleSendMessage} disabled={isLoading || !input.trim()} style={{ background: input.trim() ? 'linear-gradient(to right, #00f0ff, #ff2a2a)' : '#4B5563', opacity: input.trim() ? 1 : 1, cursor: input.trim() ? 'pointer' : 'default' }} className={`p-3 rounded-xl transition-all text-white hover:scale-105 active:scale-95`}><Send size={20} fill={input.trim() ? "currentColor" : "none"} /></button>
            </div>
