...
import { 
  ...
  User as UserIcon, Calendar, MapPin, Camera, Video, ChevronLeft, ChevronRight, Upload, PhoneOff, VideoOff, Play, Key, Globe, File as FileIcon, Plus, MessageCircle, Menu
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedSessions) {
        ...
        // Migration: Convert old history to a session
        try {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: "Failed to call the Aree API. Please try again.",
        timestamp: Date.now(),
        type: 'text'
      };
  ...
  const renderSidebar = () => (
    <>
      ...
      {/* Sidebar Content */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border-r border-gray-200 dark:border-white/10 transition-transform duration-300 md:relative md:translate-x-0 flex flex-col h-full ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        
        {/* Top Section */}
        <div className="p-4 flex flex-col gap-6 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
               <img src={BURNIT_LOGO_URL} alt="Logo" className="w-10 h-10 rounded-full animate-flame border border-burnit-cyan object-cover" onError={(e) => e.currentTarget.src = PLACEHOLDER_AVATAR} />
               <span className="font-display font-bold text-xl tracking-wide bg-gradient-to-r from-burnit-cyan to-burnit-red bg-clip-text text-transparent">Burnit</span>
            </div>
            {/* Close Button Mobile */}
          ...
          <nav className="flex flex-col gap-2">
            <button 
              ...
              className="flex items-center justify-start gap-3 p-3 rounded-xl transition-all bg-gradient-to-r from-burnit-cyan/10 to-burnit-red/10 border border-burnit-cyan/20 hover:border-burnit-cyan/50 text-black dark:text-white"
            >
              <Plus size={20} className="text-burnit-cyan" />
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-4 py-2">
           <div className="text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">History</div>
           <div className="flex flex-col gap-1">
               {sessions.map(session => (
                   <button
                      key={session.id}
                      onClick={() => switchSession(session.id)}
                      className={`flex items-center justify-start gap-3 p-2 rounded-lg transition-all text-sm truncate w-full ${currentSessionId === session.id && mode === AppMode.CHAT ? 'bg-black/5 dark:bg-white/10 text-black dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white'}`}
                   >
        ...
        {/* Bottom Section */}
        <div className="p-4 flex flex-col gap-2 shrink-0 border-t border-gray-200 dark:border-white/10 mt-auto">
           <button 
           ...
           >
              <img src={user.photoURL || PLACEHOLDER_AVATAR} alt="User" className="w-8 h-8 rounded-full border border-gray-300 dark:border-white/20 object-cover" />
  ...
  const renderProfile = () => (
      <div className="flex-1 flex flex-col h-full bg-white/50 dark:bg-black/50 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 md:p-10 pb-24 min-h-0">
            <h2 className="text-3xl font-display font-bold mb-8 text-black dark:text-white">Edit Profile</h2>
            <div className="max-w-2xl bg-white dark:bg-[#111] p-6 md:p-8 rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl">
  ...
  const renderSettings = () => (
      <div className="flex-1 flex flex-col h-full bg-white/50 dark:bg-black/50 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 md:p-10 pb-24 min-h-0">
            <h2 className="text-3xl font-display font-bold mb-8 text-black dark:text-white">Settings</h2>
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative overflow-hidden h-full">
          ...
          <div className="z-10 text-center max-w-lg">
              <div className="relative inline-block mb-8">
                  <div className="absolute inset-0 bg-burnit-red blur-2xl opacity-40 animate-pulse"></div>
                  <img src={BURNIT_LOGO_URL} className="w-32 h-32 rounded-full relative z-10 border-4 border-burnit-cyan animate-flame object-cover" onError={(e) => e.currentTarget.src = PLACEHOLDER_AVATAR} />
              </div>
  ...
  const renderLiveClass = () => (
      <div className="flex-1 flex flex-col h-full bg-gray-900 dark:bg-[#0a0a0a] p-2 md:p-4 gap-4 overflow-hidden">
          <div className="flex-1 flex flex-col md:flex-row gap-4 h-full overflow-hidden">
              <div className="flex-1 bg-black rounded-2xl border border-white/10 relative overflow-hidden flex flex-col items-center justify-center min-h-[40%]">
                  <div className="absolute top-4 left-4 bg-black/60 px-3 py-1 rounded-full text-xs font-bold text-burnit-cyan border border-burnit-cyan/20 flex items-center gap-2 z-20">
                      <div className="w-2 h-2 bg-burnit-cyan rounded-full animate-pulse"></div>
                      BURNIT AI (HOST)
                  </div>
                  <div className="relative flex flex-col items-center">
                       <div className="absolute inset-0 bg-burnit-red blur-[60px] opacity-40 animate-pulse-slow"></div>
                       <img src={BURNIT_LOGO_URL} className="w-32 h-32 md:w-56 md:h-56 rounded-full border-4 border-burnit-cyan/50 relative z-10 animate-flame object-cover shadow-[0_0_50px_rgba(0,240,255,0.3)]" onError={(e) => e.currentTarget.src = PLACEHOLDER_AVATAR} />
                       {/* REMOVED LOADING BARS AS REQUESTED */}
                  </div>
              </div>
              <div className="flex-1 bg-gray-800 dark:bg-[#111] rounded-2xl border border-white/10 relative overflow-hidden flex flex-col items-center justify-center group min-h-[40%]">
                  <div className="absolute top-4 left-4 bg-black/60 px-3 py-1 rounded-full text-xs font-bold text-white border border-white/10 z-20">YOU</div>
                  {isVideoEnabled ? (
          ...
          <div className="shrink-0 p-4 md:p-6 bg-gradient-to-t from-gray-100 via-gray-100/80 to-transparent dark:from-black dark:via-black/80 dark:to-transparent">
            <div className="relative flex items-center gap-2 max-w-4xl mx-auto bg-white dark:bg-white/5 p-2 rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl">
              ...
              <button 
                ...
                className={`p-3 rounded-xl transition-all bg-gradient-to-r from-burnit-cyan to-burnit-red text-white hover:scale-105 active:scale-95`}
              >
